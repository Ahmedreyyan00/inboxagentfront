...
function Feature({
  ...
  return (
    ...
    >
      ...
      >
        {feature.iconImage ? (
          <Image src={feature.iconImage} alt="" width={36} height={36} className="h-9 w-9 object-contain" />
        ) : (
...
function FeaturesMobile() {
  ...
  const localized = (t.secondaryItems || []).map((item, idx) => ({
    name: item.name,
    summary: item.summary,
    description: item.description,
    image: defaultFeatures[idx]?.image || defaultFeatures[0].image,
    icon: defaultFeatures[idx]?.icon || defaultFeatures[0].icon,
    iconImage: defaultFeatures[idx]?.iconImage || defaultFeatures[0].iconImage,
  })) as Array<Feature>
  const features = localized.length ? localized : defaultFeatures
  return (
    ...
    <TabGroup className="hidden lg:mt-20 lg:block">
      {({ selectedIndex }:any) => (
        <>
          ...
          {/* <TabPanels className="relative mt-20 overflow-hidden rounded-4xl bg-slate-100 px-14 pt-12 xl:px-16">
            <div className="-mx-5 flex">
              {features.map((feature, featureIndex) => (
                ...
                >
                  <div className="w-211 overflow-hidden rounded-xl bg-slate-100 shadow-lg ring-1 shadow-slate-900/5 ring-slate-500/10">
                    <Image
                      className="w-full"
                      src={feature.image}
                      alt=""
